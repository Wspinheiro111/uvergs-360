...
const origens = [
  { canal: "E-mail segmentado", inscritos: 55, presentes: 45, receita: "R$ 31.500", conversao: "82%" },
  { canal: "SMS de recuperação", inscritos: 32, presentes: 28, receita: "R$ 19.600", conversao: "88%" },
  { canal: "Portal da Câmara", inscritos: 27, presentes: 24, receita: "R$ 16.800", conversao: "89%" },
  { canal: "Indicação", inscritos: 16, presentes: 12, receita: "R$ 8.400", conversao: "75%" },
  { canal: "Orgânico", inscritos: 12, presentes: 9, receita: "R$ 6.300", conversao: "75%" },
];
    ...
    <div className="max-w-[1500px] mx-auto">
      <section className="relative overflow-hidden rounded-[36px] bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-700 p-7 lg:p-10 text-white shadow-2xl shadow-blue-200/50">
        ...
        <div className="relative flex flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
          <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/10 border border-white/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.2em] text-cyan-200">Evento 360</span><span className="rounded-full bg-emerald-400/15 px-3 py-1.5 text-[10px] font-black text-emerald-200">Demonstração</span></div><h1 className="mt-4 max-w-4xl text-4xl font-black tracking-[-.045em] leading-[.98] lg:text-6xl">Do primeiro contato ao resultado do evento.</h1><p className="mt-5 max-w-3xl text-base leading-relaxed text-blue-100 lg:text-lg">Inteligência Artificial na Gestão Pública • 24 de setembro • Porto Alegre</p></div>
          <div className="flex flex-wrap gap-3"><Link href="/eventos/ia-gestao-publica" className="rounded-2xl bg-white px-5 py-3 text-sm font-black text-blue-900">Ver landing pública</Link><Link href="/admin/growth/journeys" className="rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-black text-white">Ver jornada</Link></div>
        </div>
        <div className="relative mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["142","inscrições"],["118","presenças"],["83,1%","show rate"],["R$ 82.600","receita bruta demo"]].map(([v,l])=><div key={l} className="rounded-[24px] border border-white/10 bg-white/10 p-5 backdrop-blur"><p className="text-3xl font-black">{v}</p><p className="mt-2 text-xs text-blue-100">{l}</p></div>)}</div>
      </section>
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
        ...
        <article className="rounded-[32px] bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-6 ring-1 ring-blue-100 lg:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.2em] text-blue-700">Valor institucional</p><h2 className="mt-2 text-3xl font-black tracking-tight">Receita é importante. Relacionamento é o ativo.</h2></div><span className="rounded-full bg-blue-700 px-3 py-1.5 text-[10px] font-black text-white">UVERGS 360</span></div><p className="mt-4 max-w-4xl text-sm leading-relaxed text-slate-500">O painel permite medir resultado financeiro sem reduzir o evento a faturamento. Novos participantes, Câmaras representadas, regiões alcançadas e recorrência passam a fazer parte da leitura de sucesso.</p><div className="mt-6 grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-white p-5 ring-1 ring-blue-100"><p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Receita demo</p><p className="mt-2 text-2xl font-black text-blue-900">R$ 82,6 mil</p><p className="mt-1 text-xs text-slate-400">118 × R$ 700</p></div><div className="rounded-2xl bg-white p-5 ring-1 ring-blue-100"><p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Novos vínculos</p><p className="mt-2 text-2xl font-black text-blue-900">31</p><p className="mt-1 text-xs text-slate-400">primeira participação</p></div><div className="rounded-2xl bg-white p-5 ring-1 ring-blue-100"><p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Câmaras</p><p className="mt-2 text-2xl font-black text-blue-900">76</p><p className="mt-1 text-xs text-slate-400">representadas</p></div></div></article>
      </section>

      <section className="mt-6 rounded-[32px] border border-amber-100 bg-amber-50 p-5"><p className="text-xs leading-relaxed text-amber-900"><b>Importante:</b> todos os números desta tela são sintéticos e existem exclusivamente para demonstrar como o UVERGS 360 poderá atribuir alcance, inscrição, presença e resultado. Não representam métricas reais da UVERGS.</p></section>
      <footer className="mt-8 border-t border-slate-200 py-5 text-xs text-slate-400">UVERGS 360 • Evento 360 • Atribuição e resultados demonstrativos</footer>
    </div>
  </div>;
}
