...
const eventos = [
  { titulo: "Inteligência Artificial na Gestão Pública", data: "24 SET", local: "Porto Alegre", afinidade: "96%", status: "Inscrição confirmada", href: "/portal/vereador/credencial/ia-gestao-publica" },
  { titulo: "Comunicação Pública e Mandato", data: "17 OUT", local: "Santa Maria", afinidade: "88%", status: "Vagas abertas", href: "/eventos/ia-gestao-publica" },
  { titulo: "Gestão Municipal 360", data: "06 NOV", local: "Passo Fundo", afinidade: "81%", status: "Em breve", href: "#" },
];
    ...
    <div className="min-h-screen bg-[#f5f9ff] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-blue-100 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1450px] items-center gap-4 px-5 py-4 lg:px-10">
          ...
          <div className="flex-1"><p className="text-sm font-black tracking-tight text-blue-950">Meu UVERGS</p><p className="text-[11px] text-slate-500">Seu relacionamento com a UVERGS em um só lugar</p></div>
      ...
      <main className="mx-auto max-w-[1450px] px-5 py-8 lg:px-10 lg:py-12">
        <section className="relative overflow-hidden rounded-[36px] bg-gradient-to-br from-blue-800 via-blue-700 to-cyan-500 p-7 text-white shadow-2xl shadow-blue-200/60 lg:p-10">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_360px] lg:items-center">
            <div><p className="text-xs font-bold uppercase tracking-[.22em] text-blue-100">Olá, Ana 👋</p><h1 className="mt-3 max-w-3xl text-4xl font-black tracking-[-.04em] lg:text-6xl">Seu mandato mais conectado à UVERGS.</h1><p className="mt-5 max-w-2xl text-base leading-relaxed text-blue-100 lg:text-lg">Eventos, credenciais, certificados, histórico e oportunidades recomendadas de acordo com seus interesses.</p><div className="mt-7 flex flex-wrap gap-3"><a href="#eventos" className="rounded-2xl bg-white px-5 py-3 text-sm font-bold text-blue-800 shadow-lg">Ver próximos eventos</a><Link href="/portal/vereador/credencial/ia-gestao-publica" className="rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-bold text-white backdrop-blur">Abrir minha credencial</Link></div></div>
            <div className="grid grid-cols-2 gap-3">{[["91","afinidade UVERGS"],["4","eventos no histórico"],["3","certificados"],["82%","perfil completo"]].map(([v,l])=><div key={l} className="rounded-[26px] border border-white/15 bg-white/10 p-5 backdrop-blur-xl"><p className="text-3xl font-black">{v}</p><p className="mt-2 text-xs leading-relaxed text-blue-100">{l}</p></div>)}</div>
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
          <article className="rounded-[30px] border border-blue-100 bg-white p-6 shadow-sm lg:p-8">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><p className="text-[10px] font-bold uppercase tracking-widest text-blue-500">Tema</p><p className="mt-2 font-bold text-blue-950">IA + Gestão Pública</p></div><div className="rounded-2xl bg-emerald-50 p-4"><p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Situação</p><p className="mt-2 font-bold text-emerald-900">Confirmada</p></div><div className="rounded-2xl bg-cyan-50 p-4"><p className="text-[10px] font-bold uppercase tracking-widest text-cyan-600">Próximo passo</p><p className="mt-2 font-bold text-cyan-900">Apresentar QR</p></div></div>
            <div className="mt-6 flex flex-wrap gap-3"><Link href="/portal/vereador/credencial/ia-gestao-publica" className="rounded-2xl bg-blue-700 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-100">Abrir credencial QR →</Link><Link href="/eventos/ia-gestao-publica" className="rounded-2xl border border-blue-200 px-5 py-3 text-sm font-bold text-blue-700">Ver página do evento</Link></div>
          </article>

          <article id="perfil" className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm lg:p-8"><div className="flex items-center justify-between"><div><p className="text-[11px] font-black uppercase tracking-[.2em] text-slate-400">Meu perfil</p><h2 className="mt-2 text-xl font-black">82% completo</h2></div><div className="flex h-16 w-16 items-center justify-center rounded-full border-[7px] border-blue-100 border-t-blue-600 text-sm font-black text-blue-700">82%</div></div><p className="mt-5 text-sm leading-relaxed text-slate-500">Quanto melhor seu perfil, mais relevantes ficam os eventos, conteúdos e comunicações recomendados.</p><div className="mt-5 flex flex-wrap gap-2">{["IA","Gestão pública","Comunicação","Liderança"].map((tag)=><span key={tag} className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-bold text-blue-700">{tag}</span>)}</div><button className="mt-6 w-full rounded-2xl border border-blue-200 px-4 py-3 text-sm font-bold text-blue-700">Revisar meus dados</button></article>
        </section>
